...
article_IF_citations <- read_csv("data/articles.csv") %>% 
  ...
  # Join citing articles
  left_join(read_csv("data/citing_articles.csv"), 
            by=c("ARTICLE_DOI" = "CITED_ARTICLE_DOI")) %>%
  # Join IF data on journal title and year
...
summary <- read_csv("data/articles.csv") %>%
  ...
  # If no IF is found (e.g. in recently launched journals), IF is set to 0
  mutate(YEAR = as.numeric(format(ARTICLE_CREATED_DATE, "%Y"))) %>%
  ...
  select(-GENDER) %>%
  # Remove columns that have missing values for gender
