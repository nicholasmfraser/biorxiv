...
# Pre-allocate data table columns
all_preprints_metadata <- data.table(
  preprint_doi = character(),
  preprint_title = character(),
  preprint_posted_date = character(),
  preprint_has_published_article = logical()
)
...
for (i in 1:months){
  ...
  for (j in 1:loops) {
    ...
    all_preprints_metadata <- rbind(all_preprints_metadata, df, fill=TRUE)
...
for (i in 1:len){
  ...
  if(!(length(data))){
    ...
    # Extract created date - we use the crossref 'created-date' property as an indicator of when the article was first available through the journal page (see Haustein et al.: https://arxiv.org/ftp/arxiv/papers/1505/1505.00796.pdf)
...
rm(startdate, enddate, issn, control_doi, control_title, control_issue, control_volume, control_journal, control_publisher, df, pb, len, request)
...
rm(sub, drops, sub_merged, impact, published_articles, control_articles)
...
write.csv(published_articles, file="data/bioRxiv_preprints_published_articles_metadata.csv", row.names=FALSE)
write.csv(control_articles, file="data/bioRxiv_control_articles_metadata.csv", row.names=FALSE)
rm(preprint_altmetrics, published_altmetrics, control_altmetrics)
```
