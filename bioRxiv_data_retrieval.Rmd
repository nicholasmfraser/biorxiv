...
preprints_cr <- read_csv(file="data/preprints_cr.csv")

# Subset preprints without a matching paper in Crossref for WoS matching
cr_preprints_wos_matching <- preprints_cr %>%
...
matches <- left_join(published_categories, control_categories, by=c("FK_ISSUES", "CLASSIFICATION")) %>%
  ...
  top_n(1) %>%
  sample_n(1) %>%
  rename(PK_ITEMS_PUBLISHED = PK_ITEMS.x, PK_ITEMS_CONTROL = PK_ITEMS.y) %>%
  select(PK_ITEMS_PUBLISHED, PK_ITEMS_CONTROL)

# Write data to csv
write.csv(matches, file="data/KB/input/control_multidisciplinary_matches.csv", row.names=FALSE)

rm(matches, published_data, control_data, published_distinct, control_distinct, control_categories, published_categories)

```

### Extract full WoS records for the set of matched control articles

Input: data/KB/input/control_multidisciplinary_matches.csv -> BIORXIV_MULTI_MATCHES
SQL: published_control_multidisciplinary_matches.sql
Output: data/KB/out/control_multidisciplinary_articles.csv
...
multi_cat_articles <- read_csv("data/KB/output/control_multidisciplinary_articles.csv")

# Prepare data for matching
single_cat <- single_cat %>%
  select(-c(FK_SOURCES.x, PK_CLASSIFICATIONS, CLASSIFICATION)) %>%
  rename(PK_ITEMS_PUBLISHED = PK_ITEMS.x, PK_ITEMS = PK_ITEMS.y, FK_SOURCES= FK_SOURCES.y) %>%
  mutate(SOURCE_CATEGORY = "Single discipline")

multi_cat_articles <- multi_cat_articles %>%
  mutate(SOURCE_CATEGORY = "Mutidisciplinary")

control_articles <- bind_rows(multi_cat_articles, single_cat)

write.csv(control_articles, file="data/control_articles.csv", row.names=FALSE)

rm(single_cat, multi_cat_articles)
```
...
published_articles_final <- published_articles %>%
  mutate(TYPE = "published") %>%
  select(-c(CR_PREPRINT_DOI)) %>%
  filter(PK_ITEMS %in% control_articles$PK_ITEMS_PUBLISHED)

control_articles_final <- control_articles %>%
  mutate(TYPE = "control") %>%
  select(-c(PK_ITEMS_PUBLISHED, SOURCE_CATEGORY))

write.csv(published_articles_final, file="data/published_articles_final.csv", row.names=FALSE)
write.csv(control_articles_final, file="data/control_articles_final.csv", row.names=FALSE)


### There are NA values appearing in control articles. Why? Deleted manually for now. Need to check.
...
Published articles:
Input: data/published_articles_final.csv -> BIORXIV_PUBLISHED_FINAL
SQL Query: published_citing_articles.sql
Output: data/KB/output/published_citing_articles.csv

Control articles:
Input: data/control_articles_final.csv -> BIORXIV_CONTROL_FINAL
SQL Query: control_citing_articles.sql
Output: data/KB/output/control_citing_articles.csv


### Retrieve Crossref publication dates for all items

```{r results='hide'}
# Published articles
published_data <- cr_works_(dois = published_articles_final$DOI, .progress="text", parse=T)

# Control articles
control_data <- cr_works_(dois = control_articles_final$DOI, .progress="text", parse=T)
...
published_citing_articles <- read_csv("data/KB/output/published_citing_articles.csv")
control_citing_articles <- read_csv("data/KB/output/control_citing_articles.csv")

published_citing_data <- cr_works_(dois = published_citing_articles$DOI, .progress="text", parse=T)
control_citing_data <- cr_works_(dois = control_citing_articles$DOI, .progress="text", parse=T)
